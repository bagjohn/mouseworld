...
var GridVisualization = function(width, height, gridWidth, gridHeight, context) {
	...
	// Calls the appropriate shape(agent)
		...
		for (var i in portrayalLayer) {
            ...
            else if (p.Shape == "mouse")
                this.drawMouse(p.x, p.y, p.header, p.antenna_length, p.antenna_angle, p.r, p.Color, p.Filled, p.text, p.text_color);
		}
	};
...
/**
	Draw a circle with antennas in the specified grid cell.
	x, y: Grid coords
	s: Scaling of the circle with respect to cell size[0, 1]
	color: Color for the shape
	fill: Boolean, whether to fill or not.
        text: Inscribed text in shape.
        text_color: Color of the inscribed text.
	*/
      this.drawMouse = function(x, y, header, antenna_length, antenna_angle, radius, color, fill, text, text_color) {
            var cx = (x + 0.5) * cellWidth;
            var cy = (y + 0.5) * cellHeight;
            var r = radius * maxR;
			...
			left_antenna_header = (header + antenna_angle);
            right_antenna_header = (header - antenna_angle);
            left_antenna_x = cx + Math.cos(left_antenna_header) * antenna_length * cellWidth;
			left_antenna_y = cy + Math.sin(left_antenna_header) * antenna_length * cellHeight;
            right_antenna_x = cx + Math.cos(right_antenna_header) * antenna_length * cellWidth;
        	right_antenna_y = cy + Math.sin(right_antenna_header) * antenna_length * cellHeight;
		/**  
		    context.beginPath();
			context.arc(cx, cy, r, 0, Math.PI * 2, false);
			context.closePath();
     */	           
            context.beginPath(); 
				...
				context.lineTo(left_antenna_x, left_antenna_y);
		  		context.lineTo(cx, cy);
				context.lineTo(right_antenna_x, right_antenna_y);		
            context.closePath();
